  ...
  return (
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white rounded-xl p-6 shadow-sm">
        <div>
          <Label className="text-[#0A1628] font-medium">Título principal *</Label>
          <Input {...register("title", { required: true })} className="mt-1.5" />
        </div>

        <div>
          <Label className="text-[#0A1628] font-medium">Subtítulo / Tagline *</Label>
          <Textarea {...register("subtitle", { required: true })} rows={3} className="mt-1.5 resize-none" />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <Label className="text-[#0A1628] font-medium">Texto del botón *</Label>
            <Input {...register("ctaText", { required: true })} className="mt-1.5" />
          </div>
          <div>
            <Label className="text-[#0A1628] font-medium">Enlace del botón *</Label>
            <Input {...register("ctaLink", { required: true })} placeholder="#contacto" className="mt-1.5" />
          </div>
        </div>

        <div>
          <Label className="text-[#0A1628] font-medium">Imagen de fondo (opcional)</Label>
